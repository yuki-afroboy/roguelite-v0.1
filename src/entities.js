  ...
  update(dt, G) {
    ...
    for (let i = 0; i < this.cap; i++) {
      ...
      if (d.ai === 'dash') {
        ...
        // 一度ここに後退挙動を入れたことがあり、近づくと逃げていく＝
        // 手応えのない群れになっていた。跳びかかりは「速くなる」だけで、
        // 離れる動きは持たせない。
      ...
      // 接触。触れたら減る。種類も跳びかかり中かも問わない。
      //
      // かつて「跳びかかった個体だけが痛い」という規則を置いていたが、
      // 正面から体当たりしても何も起きず、壊れているようにしか見えなかった。
      // 削られ過ぎる問題は、無敵時間・同時に跳びかかれる数の上限・最大HP・
      // レベルアップ回復で別に解いてあるので、この規則はもう要らない。
      //
      // 針は獣を押しのけない（すり抜ける）。押し出すと体当たりが成立しない。
